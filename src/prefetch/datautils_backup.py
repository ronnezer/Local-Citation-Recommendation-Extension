...
class PrefetchDataset( Dataset ):
    ...
    def load_document(self, paper_id, is_citing_document=False):
        ...
        year = paper.get("year", None)  # Extract year
        if is_citing_document:
            document = [[title, self.citation_title_label, year],
                        [abstract, self.citation_abstract_label, year]]
        else:
            document = [[title, self.document_title_label, year],
                        [abstract, self.document_abstract_label, year]]
        return document
    ...
    def load_citation_context(self, context_id):
        ...
        # Extracting the document including the year
        citing_document = self.load_document(citing_id, is_citing_document=True)
        # Assuming the year is the third element in each entry of the document
        # and extracting it for the citation context (this may need to be adjusted)
        year_of_citing_document = citing_document[0][2]

        citation_context_document = citing_document + [
            [context_text, self.citation_context_label, year_of_citing_document]]
    ...
    def encode_document(self, document):
        document = document[:self.max_doc_len]
        document = document + [[ "", self.padding_paragraph_label, None] for _ in range(self.max_doc_len - len(document))]
        ...
        paragraph_text_list, paragraph_type_list, year_list, paragraph_mask_list = list(zip(*document))
        paragraph_seq_list = [self.vocab.sent2seq(para, self.max_seq_len) for para in paragraph_text_list]
        return paragraph_seq_list, paragraph_type_list, year_list, paragraph_mask_list
